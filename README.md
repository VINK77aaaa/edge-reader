...
- **文档拖入即读**：把 `docx` / `doc` / `txt` / `md` 文件拖进窗口即可加载正文并朗读（.doc 需本机装有 Word；也可点「📂 打开文档」选择文件）
...
- **历史记录**：文章自动保存（默认折叠），支持删除
- **继续上次阅读**：恢复上次阅读的位置和进度百分比
- **快捷键**：`空格` 播放/暂停 · `Esc` 返回 · `←`/`→` 上一句/下一句
- **其他**：记住窗口大小与位置、窗口标题跟随文章、朗读条显示合成/播放进度、雅黑/宋体切换
